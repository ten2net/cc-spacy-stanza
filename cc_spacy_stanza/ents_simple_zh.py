...
ner_lst = nlp_spacy.pipe_labels['ner']
print("ner label:===========================================================")
ner_lst = nlp_spacy.pipe_labels['ner']
print(len(ner_lst))
print(ner_lst)

# nlp = spacy_stanza.load_pipeline("zh-hans",tokenize_pretokenized=True)

# 1、api测试

doc = nlp("七七事变，又称卢沟桥事变，发生于1937年7月7日 [1] 。1937年7月7日夜，卢沟桥的日本驻军在未通知中国地方当局的情况下，径自在中国驻军阵地附近举行所谓军事演习，并诡称有一名日军士兵失踪，要求进入北平西南的宛平县城搜查 [13] ，被中国驻军严词拒绝，日军随即向宛平城和卢沟桥发动进攻。中国驻军第29军37师219团奋起还击，进行了顽强的抵抗 [2] 。“七七事变”揭开了全国抗日战争的序幕.")
...
# print("doc.to_array([LOWER, POS, ENT_TYPE, IS_ALPHA]):=======================================================")
# print(np_array)
# np_array = doc.to_array("POS")
# print("doc.to_array(POS):=======================================================")
# print(np_array)
print(doc.to_json())
...
for doc in documents:
    ...
    # 提取命名实体、类型和位置信息，并将结果存储在列表中
    for ent in doc.ents:
        entities_list.append(ent.text)
        labels_list.append(ent.label_)
        start_list.append(ent.start_char)
        end_list.append(ent.end_char)
...
# 导出为Excel文件
# data.to_excel("命名实体识别结果.xlsx", index=False)
data.to_csv("命名实体识别结果.csv")
...
# 创建空的数据框
data = pd.DataFrame(columns=["卷宗", "时间", "类型", "事件", "位置"])
...
data.to_csv("命名实体信息.csv", index=False) 
...
# 3. GPE(地理/社会/政治实体,Geo-Political)
# 4. LOC(处所,Locations)
# 5. FAC(设施,Facilities)
# 6. VEH(交通工具,Vehicle)
# 7. WEA(武器,Weapon)
